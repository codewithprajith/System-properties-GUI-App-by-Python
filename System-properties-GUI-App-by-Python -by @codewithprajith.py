...
Label(root, text='©codewithprajith', font='Verdana 13 bold', fg='#0d0d0d', bg="#ffffff").pack()
TextBox = Text(root, height=15, width=57, font='Verdana 10 bold', bg='#c4e0e5')
...
def SystemInfo():
    ...
    TextBox.insert(END, '_____________________System Information___________________\n\n')
...
def boottime():
    TextBox.delete('1.0', END)
    TextBox.insert(END, '_______________________BOOT TIME_______________________\n\n')
    boot_time_timestamp = psutil.boot_time()
    bt = datetime.fromtimestamp(boot_time_timestamp)
    TextBox.insert(END, f"Last Boot Time: {bt.year}/{bt.month}/{bt.day} {bt.hour}:{bt.minute}:{bt.second}")
...
def CPUInfo():
    ...
    TextBox.insert(END, f" Charger Plugged-in: {info[2]}")
...
def DiskInfo():
    TextBox.delete('1.0', END)
    TextBox.insert(END, '_____________________DISK INFORMATION____________________\n\n')
...
def NetworkInfo():
    TextBox.delete('1.0', END)
    TextBox.insert(END, '__________________NETWORK INFORMATION_________________\n\n')
...
def GPUInfo():
    TextBox.delete('1.0', END)
    TextBox.insert(END, '______________________GPU INFORMATION___________________\n\n')
    gpus = GPUtil.getGPUs()
    for gpu in gpus:
        # get the GPU id
...
def Followme():
   webbrowser.open('https://www.instagram.com/_prajith_kumar.__/')
Button(root, text='System Info', bg='#aed1cb', fg='#0e1117', font='Verdana 10 bold', width=10, command=SystemInfo).place(x=20, y=100)
Button(root, text='Boot Time', bg='#aed1cb', fg='#0e1117', font='Verdana 10 bold', width=10, command=boottime).place(x=140, y=100)
Button(root, text='CPU Info', bg='#aed1cb', fg='#0e1117', font='Verdana 10 bold', width=10, command= CPUInfo).place(x=260, y=100)
Button(root, text='Memory Info', bg='#aed1cb', fg='#0e1117', font='Verdana 10 bold', width=10, command=MemoryInfo).place(x=380, y=100)
Button(root, text='Disk Info', bg='#aed1cb', fg='#0e1117', font='Verdana 10 bold', width=10, command=DiskInfo).place(x=500, y=100)
Button(root, text='Network Info', bg='#aed1cb', fg='#0e1117', font='Verdana 10 bold', width=10, command = NetworkInfo).place(x=20, y=160)
Button(root, text='GPU Info', bg='#aed1cb', fg='#0e1117', font='Verdana 10 bold', width=10, command=GPUInfo).place(x=20, y=220)
Button(root, text='Follow me \n on Instagram', bg='#aed1cb', fg='#0e1117', font='Verdana 10 bold', width=10, command=Followme).place(x=20, y=280)
